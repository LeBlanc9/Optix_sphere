    ...
    const phonder::DevicePhotonBatch& input_batch) // Now accepts pre-existing batch
{
    ...
    // 2. Prepare device buffers for statistics
    ...
    // 'flux' directly represents the sum of weights of detected photons,
    // so it's already the detected power in Watts. No further normalization needed.

    SimulationResult result;
    result.total_rays = photon_count;
    result.detected_rays = detected_rays;
    result.detected_flux = flux;
