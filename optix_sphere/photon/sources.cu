...
void FocusedSpotSource::generate(PhotonBatch& batch, int num_photons, unsigned long long seed) const {
    ...
    generate_focused_spot_source_kernel<<<grid_size, block_size>>>(
        ...
        batch.directions(),
        batch.weights(),
        num_photons, spot_center, spot_radius, disk_normal,
        convergence_half_angle_rad, main_axis, weight, seed
    );
    cudaDeviceSynchronize();
}

void LambertianDiskSource::generate(PhotonBatch& batch, int num_photons, unsigned long long seed) const {
    batch.resize(num_photons);
    const int block_size = 256;
    const int grid_size = (num_photons + block_size - 1) / block_size;
    generate_lambertian_disk_source_kernel<<<grid_size, block_size>>>(
        batch.positions(),
        batch.directions(),
        batch.weights(),
        num_photons, center_position, disk_normal, radius, weight, seed
    );
    cudaDeviceSynchronize();
}
